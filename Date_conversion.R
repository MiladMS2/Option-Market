...
m2s = function(date) {
  date <- date %>% toString() %>% str_split_1(pattern = "[-/]")
  gy <- date[1] %>% as.numeric()
  gm <- date[2]%>% as.numeric()
  gd <- date[3]%>% as.numeric()
  ...
  return(paste(c(jy, sprintf("%02d",jm), sprintf("%02d",jd)),collapse = "/"))
}

s2m = function(date) {
  date <- date %>% toString() %>% str_split_1(pattern = "[-/]")
  jy <- date[1] %>% as.numeric()
  jm <- date[2] %>% as.numeric()
  jd <- date[3] %>% as.numeric()
  jy = jy + 1595
  ...
  return(paste(c(gy, sprintf("%02d",gm), sprintf("%02d",gd)),collapse = "/"))
}
